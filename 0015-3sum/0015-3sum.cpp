// Method 1: Sorting and then two pointer approach
/*
        ...
        for(int i=0; i<n; i++){
            ...
            while(left < right){
                if(nums[left] + nums[right] == remaining){
                    ans.push_back({val1, nums[left], nums[right]});
                    left++;
                    right--;
                    ...
                    // skipping duplicate right values
...
// Method 2: Similar approach without sorting (HashSet based)
/*
...
//         unordered_set<string> triplets; // global deduplication
//         unordered_set<int> usedA;       // avoid processing same anchor value again
...
//             // If this anchor value was already processed, skip it
...
//             unordered_set<int> seen; // numbers seen for this fixed anchor a

//             for (int j = i + 1; j < n; j++) {
//                 int b = nums[j];
//                 int c = -a - b;

//                 // If c was already seen, then a + b + c = 0
//                 if (seen.find(c) != seen.end()) {
//                     int x = a;
//                     int y = b;
//                     int z = c;

//                     // Canonicalize triplet into sorted order: mn, mid, mx
...
//                     // Add only if this exact triplet has not appeared before
//                     if (triplets.find(key) == triplets.end()) {
//                         triplets.insert(key);
//                         ans.push_back({mn, mid, mx});
//                     }
//                 }

//                 // Add b after checking, so we do not reuse the same index
//                 seen.insert(b);
//             }
//         }
