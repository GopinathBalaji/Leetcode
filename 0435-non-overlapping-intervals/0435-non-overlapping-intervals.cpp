// Method 1: Greedy Approach sorting by start
/*
################ WHAT WAS I DOING WRONG ##################
The main logic bug is this:

When you detect an overlap, you do:

```cpp
count++;
```

but you **do not update `back`**.

That means you are always keeping the earlier interval, even if it ends later and is a worse choice.

For this problem, when two intervals overlap, you should keep the one with the **smaller end**, because that leaves more room for future intervals.

---

## Why your code fails

Your code sorts by start time and keeps:
...
* `back = [1,100]`
* compare `[11,12]` with `[1,100]` → overlap, so `count = 1`
* compare `[12,13]` with `[1,100]` → overlap again, so `count = 2`

Answer becomes `2`.

### But optimal answer is `1`
...
So only **one** interval needs to be removed.

---

## What should happen on overlap

When intervals overlap, you must do:

* increment removal count
* keep the interval with the **smaller end**

So in the overlap case, do something like:

```cpp
back[1] = min(back[1], intervals[i][1]);
```

or more explicitly choose the better interval.

Because among overlapping intervals:

* smaller end is always better
* larger end blocks more future intervals



## Even cleaner greedy idea

The standard solution is to sort by **end time**, not start time.

Why? Because then you always keep the interval that finishes earliest.

That makes the greedy logic much more natural.

---

## In one sentence

Your solution counts overlaps, but on overlap it keeps the wrong interval; you must keep the interval with the **smaller ending time**, otherwise you can overcount removals.

If you want, I can also show why the **sort by end time** greedy is the most natural solution for 435.
##########################################################
*/
class Solution {
public:
...
// Method 2: Greedy Approach sorting by end
/*
...
//         // 2) dp[i] = max number of non-overlapping intervals
//         // among intervals[0..i]
//         vector<int> dp(n, 0);
//         dp[0] = 1;  // with the first interval, best is to take it
...
//             int take = 1;  // take this interval alone if nothing before fits

//             // Find best j < i that does not overlap with interval i
//             for (int j = i - 1; j >= 0; j--) {
//                 if (intervals[j][1] <= intervals[i][0]) {
//                     // intervals[j] ends before or at the start of intervals[i]
//                     take = max(take, 1 + dp[j]);
//                     break;  // first such j from the back is optimal
//                 }
//             }
